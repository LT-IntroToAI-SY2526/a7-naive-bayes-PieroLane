...
class BayesClassifier:
    ...
    def classify(self, text: str) -> str:
        ...
        # of 0)
        for token in tokens:
            pos_tokenfreq = self.pos_freqs.get(token, 0) + 1
            pos_score += math.log(pos_tokenfreq / pos_total)
            print(f"word: {token}, pos_score running total: {pos_score}")
            
        for token in tokens:
            neg_tokenfreq = self.neg_freqs.get(token, 0) + 1
            neg_score += math.log(neg_tokenfreq / neg_total)
